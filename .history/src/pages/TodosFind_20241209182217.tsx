...
const TodosFind = () => {
  ...
  const { categoryNames, setCategoryName, filteredTodos,categoryName:selectedCategoryName } =
  ...
  return (
    <MobileLayout>
      ...
      <CategoryBar categoryNames={categoryNames} onClick={setCategoryName} selectedCategoryName={selectedCategoryName} />
...
export const CategoryBar = ({ selectedCategoryName,categoryNames, onClick }: CategoryBarProps) => {
  const theme = useTheme()
  return (
    <Box sx={{ display: 'flex', overflowX: 'auto', gap: '12px' }}>
      {categoryNames.map((categoryName, idx) => (
        <Button
          style={{
            width: 'auto',
            padding: '6px, 12px',
            background: categoryName === selectedCategoryName ? theme.palette.gray1000 : theme.palette.gray300,
            borderRadius: '4px',
            color: theme.palette.
          }}
